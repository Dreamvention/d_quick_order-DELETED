...
class ModelExtensionModuleDQuickOrder extends Model
{
    ...
    public function createOrdersHistoryTable()
    {
        $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "d_qo_order_history` (
            `order_history_id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY, 
            `quick_order_id` int(11) NOT NULL,
    ...
    public function setRealOrderHistory($order_id, $order_status_id, $notify, $comment)
    {
        return $this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "', date_added = NOW()");
    }

    public function setRealOrderTotalSubtotal($order_id, $code, $title, $value, $sort_order)
    {
        return $this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int)$order_id . "', code = '" . $this->db->escape($code) . "', title = '" . $this->db->escape($title) . "', value = '" . (float)$value . "', sort_order = '" . (int)$sort_order . "'");
    }
